...
import { removeStockTransferFromGodown } from "../../../../slices/voucherSlices/commonVoucherSlice";

function AddGodownTile() {
  const selectedStockTransferFromGodown = useSelector(
    (state) => state.commonVoucherSlice?.stockTransferFromGodown?.godown || ""
  );
      ...
      <div className="bg-white  py-3 px-4 pb-3 drop-shadow-lg mt-2 md:mt-3 text-xs md:text-base ">
        <div className="flex justify-between">
          <div className="flex gap-2 ">
            <p className="font-bold uppercase text-xs">From Godown / Van</p>
            <span className="text-red-500 mt-[-4px] font-bold">*</span>
          </div>

          {selectedStockTransferFromGodown && (
        ...
        {!selectedStockTransferFromGodown ? (
          ...
          <div className="mt-3 p-3 py-2 border  border-gray-300  rounded-md   cursor-pointer items-center font-medium flex justify-between gap-4">
            <div className="flex justify-center items-center gap-3">
              ...
              <span>{selectedStockTransferFromGodown}</span>
            </div>
            <div>
              {/* <Link to={link}> */}
              <p
                onClick={() => {
                  dispatch(removeStockTransferFromGodown());
                }}

    ...
    <div className="rounded-lg overflow-hidden shadow-sm w-full max-w-full pb-5">
        ...
        <div className="space-y-2 text-xs">
          ...
          <div className="pt-2 mt-1 border-t border-gray-100">
            <div className="flex items-center justify-between font-bold">
              <p>Total Amount</p>
              <p className="text-sm sm:text-base">₹ {parseInt(data?.finalAmount).toFixed(2)}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="mt-2 bg-white rounded-lg overflow-hidden shadow-sm">
          ...
          >
            <div className="flex items-center gap-1 text-xs sm:text-sm font-bold text-gray-800">
              {showItems ? <IoIosArrowUp size={14} /> : <IoIosArrowDown size={14} />}
                  ...
                  <div className="text-xs text-gray-500">
                    ...
                    {item.hasGodownOrBatch ? (
                      <div className="mt-1 space-y-2">
                        {item.GodownList.filter(g => g.added).map((godownOrBatch, idx) => (
                          <div key={idx} className="pl-2 border-l-2 border-violet-200">
                            {godownOrBatch.batch && (
                              <div className="flex flex-wrap justify-between mb-1">
                                <p className="text-violet-600 font-medium">Batch: {godownOrBatch.batch}</p>
                            ...
                            {(godownOrBatch.discount > 0 || godownOrBatch.discountPercentage > 0) && (
                              <div className="flex justify-between mb-1">
                                <p>Discount</p>
                                <p>
      ...
      {additionalCharges?.length > 0 && (
        <div className="mt-2 bg-white rounded-lg overflow-hidden shadow-sm">
          ...
          >
            <div className="flex items-center gap-1 text-xs sm:text-sm font-bold text-gray-800">
              {showCharges ? <IoIosArrowUp size={14} /> : <IoIosArrowDown size={14} />}

...
function VoucherThreeInchPdfFormat2({ data, org: orgProp, isPreview, sendToParent }) {
  ...
  let showPrintButton = org?.configurations?.[0]?.defaultPrint?.showBeforeSaleInRestaurant;
  ...
  const discountBasedOnGrossAmount = org?.configurations?.[0]?.discountBasedOnGrossAmount ?? false;
  const includeTaxWithPrint = org?.configurations?.[0]?.defaultPrint?.showPrintWithTaxInRestaurant;
  ...
  useEffect(() => {
    ...
    const taxableSubTotal = data.items.reduce(
      (acc, curr) => acc + Number(curr?.GodownList?.[0]?.taxableAmount ?? curr?.taxableAmount ?? 0),
      0,
    );
  ...
  const getItemTaxableAfterDiscount = (item, totalDiscount, grossAmount) => {
    ...
    const lineDiscount = grossAmount > 0 ? (lineBeforeTax / grossAmount) * totalDiscount : 0;
    return lineBeforeTax - lineDiscount;
  };

  const calculateTotalTaxWithDiscountLogic = () => {
    if (!data?.items?.length) return 0;
    const totalDiscount = Number(data?.totalAdditionalCharges || data?.additionalCharges?.[0]?.finalValue) || 0;
    ...
    const totalTax = data.items.reduce((acc, item) => {
      const taxableAfterDiscount = getItemTaxableAfterDiscount(item, totalDiscount, grossTaxable);
      if (isInterState) return acc + (taxableAfterDiscount * Number(item.igst || 0)) / 100;
      return acc +
  ...
  const calculateTotalTax = () =>
    discountBasedOnGrossAmount
      ? Number(data?.items?.reduce((acc, curr) =>
          acc + Number(curr?.totalIgstAmt || curr?.totalCgstAmt + curr?.totalSgstAmt || 0), 0) || 0)
      : calculateTotalTaxWithDiscountLogic();

  const getBillNumber = () =>
    data?.[getVoucherNumber()] || data?.voucherNumber?.[0]?.voucherNumber || "11007";

  const getTableNumber = () =>
    data?.voucherNumber?.map((item) => item?.tableNumber).filter(Boolean).join(", ") ||
    data?.convertedFrom?.map((item) => item?.tableNumber).filter(Boolean).join(", ") ||
    null;
  ...
  const discount = Number(data?.totalAdditionalCharges || data?.additionalCharges?.[0]?.finalValue || 0).toFixed(2);
  ...
  const generateReceiptPDFAsBase64 = async () => {
    ...
    if (isA5Mode) { element.style.border = "none"; element.style.padding = "8mm"; }

    const canvas = await html2canvas(element, {
      scale: 2, useCORS: true, backgroundColor: "#ffffff",
      logging: false, windowWidth: emailWindowWidth,
    });
    ...
    const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: emailPaperFormat });

    if (imgRenderedHeight <= availableHeight) {
      doc.addImage(imgData, "PNG", emailMargin,
        emailMargin + (availableHeight - imgRenderedHeight) / 2, availableWidth, imgRenderedHeight);
    } else {
      const scaledWidth = availableWidth * (availableHeight / imgRenderedHeight);
      doc.addImage(imgData, "PNG",
        emailMargin + (availableWidth - scaledWidth) / 2, emailMargin, scaledWidth, availableHeight);
    }
  ...
  const handleShareBill = async (option, message, ccEmails, toEmail) => {
    ...
    if (option === "Mail") {
      ...
      const res = await api.post("/api/sUsers/send-bill-email", {
        toEmail, ccEmails, message, billNo,
        guestName: data?.party?.partyName || data?.customerName || "Customer",
        organizationName: org?.name, pdfBase64,
  ...
  const handleShareClick = async () => {
    ...
    const itemsList = data?.items?.map((el, i) =>
    ...
    const { value: option } = await Swal.fire({
      title: "Share through", input: "radio",
      inputOptions: { WhatsApp: "WhatsApp", Mail: "Mail" },
      confirmButtonText: "Next", confirmButtonColor: "#000000",
      showCancelButton: true, cancelButtonText: "Cancel", cancelButtonColor: "#dd3333",
      inputValidator: (v) => !v && "Please select an option!",
    });
    if (!option) return;

    let toEmail = "", ccEmails = [];
    ...
    if (option === "Mail") {
      const { value: emailData, isDismissed } = await Swal.fire({
        ...
        showCancelButton: true, cancelButtonColor: "#dd3333",
        confirmButtonText: "Next", confirmButtonColor: "#000000",
        width: "540px", focusConfirm: false,
        preConfirm: () => {
    ...
    const { value: finalMessage, isDismissed: msgDismissed } = await Swal.fire({
      ...
      showCancelButton: true, cancelButtonColor: "#dd3333",
      confirmButtonText: "Send", confirmButtonColor: "#000000",
      width: "660px", focusConfirm: false,
      preConfirm: () => {
    ...
    try {
      ...
      Swal.fire({ icon: "error", title: "Error", text: error.message || "Failed to share.", confirmButtonColor: "#000000" });
    }
  };

  // ─── Styles ───────────────────────────────────────────────────────────────
  const thermalContainerStyle = {
    width: "72mm", margin: "0 auto", fontFamily: "Arial, sans-serif",
    fontSize: "11px", lineHeight: 1.2, padding: "2mm 3mm",
    border: "1px dotted #000", textAlign: "left", boxSizing: "border-box",
  };
  const a5ContainerStyle = {
    width: "148mm", margin: "0 auto", fontFamily: "Arial, sans-serif",
    fontSize: "12px", lineHeight: 1.4, padding: "8mm 10mm",
    textAlign: "left", boxSizing: "border-box", background: "#fff",
  };
  const containerStyle = isA5Mode ? a5ContainerStyle : thermalContainerStyle;

  const flexRow = { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2px" };
  ...
  const headerGrid = {
    display: "grid", gridTemplateColumns: "0.6fr 2.2fr 0.7fr 1fr 1.1fr",
    fontSize: isA5Mode ? "12px" : "11px", fontWeight: "bold",
    paddingBottom: "3px", borderBottom: "1px dotted #000", marginBottom: "4px",
  };
  const itemGrid = {
    display: "grid", gridTemplateColumns: "0.6fr 2.2fr 0.7fr 1fr 1.1fr",
    fontSize: isA5Mode ? "11px" : "10px", marginBottom: "2px", padding: "1px 0",
  };

  const paymentSplits = data?.paymentSplittingData || [];
  const getPaymentSummary = () => {
    if (!paymentSplits.length) return null;
    return paymentSplits.filter((p) => p.amount > 0).map((p) => (
      <>
        <div key={p.type} style={{ fontSize: "10px", fontWeight: "bold" }}>
          {p.subsource} : ₹ {Math.round(p.amount).toFixed(2)}
        </div>
        {p?.credit_reference_type && (
          <div key={p.type + "_credit"} style={{ fontSize: "10px", fontWeight: "bold" }}>
    ...
    <div ref={dropdownRef} style={{ position: "relative", display: "inline-block" }}>
      <button
        ...
        style={{
          ...
          padding: "6px 14px", border: "1.5px solid #3b82f6", borderRadius: "8px",
          background: "#fff", fontSize: "13px", fontWeight: "500",
          color: "#1e3a5f", cursor: "pointer", whiteSpace: "nowrap",
        }}
      >
      ...
      {dropdownOpen && (
        <div style={{
          ...
          background: "#1f2937", borderRadius: "8px", minWidth: "200px",
          zIndex: 999, boxShadow: "0 8px 24px rgba(0,0,0,0.25)", overflow: "hidden",
        }}>
          {Object.entries(PRINT_MODE_LABELS).map(([mode, label]) => (
            <button
              key={mode}
              onClick={() => { setPrintMode(mode); setDropdownOpen(false); }}
              style={{
  ...
  const ReceiptHeader = () => {
    ...
    if (isA5Mode && hasLogo) {
      return (
        <div style={{
          ...
          paddingBottom: "6px", borderBottom: "1px dotted #000", gap: "12px",
        }}>
          <img src={org.logo} alt="Logo"
            style={{ width: "35mm", height: "auto", objectFit: "contain", flexShrink: 0 }} />
          <div style={{ textAlign: "right" }}>
            <div style={{ ...bold, fontSize: "20px", marginBottom: "3px" }}>{org?.name}</div>
            {(org?.road || org?.place) && (
              <div style={{ fontSize: "11px" }}>
                {`${org?.road || ""}${org?.road && org?.place ? ", " : ""}${org?.place || ""}`}
              </div>
            )}
            {org?.mobile && <div style={{ fontSize: "11px" }}>PH: {org.mobile}</div>}
            {org?.gstNum && <div style={{ fontSize: "11px" }}>GSTIN: {org.gstNum}</div>}
          </div>
        </div>
      );
    }
    ...
    return (
      <div style={{
        ...flexRow, marginBottom: "8px", paddingBottom: "6px",
        borderBottom: "1px dotted #000", alignItems: "flex-start", gap: "8px",
      }}>
        {hasLogo && (
          <img src={org.logo} alt="Logo"
  ...
  const receiptJSX = (
    <div ref={contentToPrint} className="receipt-container" style={containerStyle}>
      ...
      <div style={{ ...centerText, marginBottom: "6px", paddingBottom: "6px", borderBottom: "1px dotted #000" }}>
        <div style={{ fontSize: isA5Mode ? "16px" : "14px", fontWeight: "bold", fontStyle: "italic" }}>INVOICE</div>
      </div>

      {/* Bill Info */}
      <div style={{ marginBottom: "6px", fontSize: isA5Mode ? "12px" : "11px", fontWeight: "bold", paddingBottom: "6px", borderBottom: "1px dotted #000" }}>
        ...
        {data?.party?.partyName && (
          <div style={{ marginTop: "2px" }}>
            ...
            <span style={{ textTransform: "capitalize" }}>{data?.party?.partyName}</span>
          </div>
        )}
      </div>
      ...
      {data?.items?.map((el, index) => {
        ...
        const totalTax = IsComplimentary ? 0
          : el?.totalIgstAmt != null ? Number(el.totalIgstAmt)
        ...
        <div style={{ flex: 1 }}>
          {!discountBasedOnGrossAmount && Number(discount) > 0 && (
            <>
              <div style={{ display: "flex", flexDirection: "row", marginBottom: "2px", fontWeight: "bold" }}>
                ...
                <div style={{ width: 60, textAlign: "right" }}>{(Number(subTotal || 0) + Number(discount || 0)).toFixed(2)}</div>
              </div>
              <div style={{ display: "flex", flexDirection: "row", marginBottom: "2px", fontWeight: "bold" }}>
          ...
          {!data?.isComplimentary && (
            <>
              ...
              {(org?.industry == 7 ? calculateTotalTax() > 0 : isIndian && isSameState && calculateTotalTax() > 0) && (
                <>
                  <div style={flexRow}>
                    <div style={{ marginLeft: "auto", width: 70, fontWeight: "bold" }}>CGST {cgstPercentage}%</div>
                    <div style={textRight}>{cgst}</div>
                  </div>
                  <div style={flexRow}>
                    <div style={{ marginLeft: "auto", width: 70, fontWeight: "bold" }}>SGST {sgstPercentage}%</div>
          ...
          {!org?.industry == 7 && isIndian && !isSameState && calculateTotalTax() > 0 && (
            <div style={flexRow}>
              <div style={{ marginLeft: "auto", width: 70, fontWeight: "bold" }}>IGST {igstPercentage}%</div>
              <div style={textRight}>{tax}</div>
            </div>
          )}
          {!isIndian && calculateTotalTax() > 0 && (
            <div style={flexRow}>
              <div style={{ marginLeft: "auto", width: 70, fontWeight: "bold" }}>Tax {igstPercentage}%</div>
              <div style={textRight}>{tax}</div>
            </div>
          )}
        </div>
      ...
      <div style={{ fontSize: "10px", marginBottom: "6px" }}>
        <div style={{ display: "flex", fontWeight: "bold", marginBottom: "2px" }}>
          ...
          <div style={{ marginLeft: "auto", paddingRight: "3px", fontWeight: "bold" }}>Total: {netAmount}</div>
        </div>
      ...
      {/* Net Amount */}
      <div style={{ ...centerText, fontSize: isA5Mode ? "16px" : "14px", fontWeight: "bold", marginBottom: "8px", paddingBottom: "6px", borderBottom: "1px dotted #000" }}>
        Net Amount: {netAmount}
      </div>
  ...
  return (
    <>
      ...
      <div className="grid mt-2">
        {isA5Mode ? (
          <div style={{ background: "#fff", padding: "32px 16px", minHeight: "400px", display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{ background: "#fff", boxShadow: "0 4px 24px rgba(0,0,0,0.18)", borderRadius: "2px", width: "148mm", maxWidth: "100%" }}>
              {receiptJSX}
            </div>
            <div style={{ marginTop: "12px", fontSize: "12px", color: "#6b7280" }}>Previewing A5 layout</div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            {receiptJSX}
            <div style={{ marginTop: "8px", fontSize: "12px", color: "#6b7280" }}>Previewing 80mm thermal layout</div>
          </div>
        )}

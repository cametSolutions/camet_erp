...
function InvoiceListSecondary() {
  ...
  useEffect(() => {
    ...
    const fetchInvoice = async () => {
      try {
        const res = await api.get(`/api/sUsers/invoiceList/${cpm_id}`, {
          withCredentials: true,
        });

        setTimeout(() => {
          setInvoice(res.data.invoiceList);
        }, 1000);
      } catch (error) {
        console.log(error);
      } finally {
  ...
  useEffect(() => {
    if (search === "") {
      ...
      const filtered = invoice.filter((el) =>
        el?.party?.partyName.toLowerCase().includes(search.toLowerCase())
      );
      setFilteredInvoice(filtered);
    }
  }, [search, invoice, refresh]);
  ...
  const Row = ({ index, style }) => {
    const el = filteredInvoice[index];
    return (
      <>
        <div
          key={index}
        ...
        >
          <div className="flex justify-between w-full gap-3 ">
            <div className="">
              <p className="font-bold text-sm">{el?.party?.partyName}</p>
              {el?.party?.mobileNumber && (
                <div className="flex">
...
export default InvoiceListSecondary;

...
export default function SummaryReport() {
  ...
  const totalAmount = useMemo(() => {
    return (
      processedSummary?.reduce((sum, item) => sum + Number(item.net), 0) ?? 0
    );
  }, [processedSummary]);
              ...
              {processedSummary.map((item, index) => {
                const transactionCount =
                  data?.flattenedResults
                    ...
                    ?.reduce(
                      (sum, backendItem) => sum + (backendItem.count || 0),
                      0
                    ) || 0;
                ...
                return (
                  <div
                    key={index}
                    className="bg-white shadow-sm rounded-xs p-3 hover:shadow-md transition-all duration-200 border-l-2 "
                  >
                    <div className="flex items-center justify-between gap-1">
                      <div>
                        <div className="flex items-center ">
                          <h4 className="text-sm font-bold text-gray-800 flex-shrink-0 w-24">
                            {item.name}
                          </h4>

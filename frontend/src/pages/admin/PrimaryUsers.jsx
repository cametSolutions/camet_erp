...
import { AlertCircle, Crown, Star, Building2 } from "lucide-react";
...
function PrimaryUsers() {
  ...
  const getOverviewStats = () => {
    ...
    const secondaryUsersActive = secUsers.filter(
      (user) => user.isActive
    ).length;
    const secondaryUsersInactive = secUsers.filter(
      (user) => !user.isActive
    ).length;
  ...
  const getSecondaryUserStats = (userId) => {
    const userSecUsers = secUsers.filter(
      (secUser) => secUser?.primaryUser === userId
    );
    const activeSecUsers = userSecUsers.filter((secUser) => secUser?.isActive);
    const inactiveSecUsers = userSecUsers.filter(
      (secUser) => !secUser?.isActive
    );
  ...
  const getStatusColor = (user) => {
    ...
    if (user.subscription === "yearly")
      return "bg-gradient-to-br from-blue-50 to-gray-50 border-blue-200";
        ...
        <Card className="w-full max-w-md transform transition-all duration-500 hover:scale-105">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-gray-600 mb-4">
              There was an error loading the primary users data.
            </p>
            <Button
            ...
            return (
              <Card
                ...
                className={`${getStatusColor(
                  user
                )} border rounded-xl relative overflow-hidden transition-all duration-500 shadow-lg hover:shadow-2xl hover:-translate-y-2 cursor-pointer group transform ${
                  isVisible
                    ? "opacity-100 translate-y-0"
              ...
              >
                ...
                <div
                  className={`absolute top-4 right-4 w-3 h-3 rounded-full transition-all duration-300 ${
                    ...
                    isHovered
                      ? "animate-pulse scale-125 shadow-lg"
                      : "shadow-md"
                  }`}
                />
                  ...
                  <div className="text-xs text-gray-500 space-y-1 bg-gray-50 py-2 px-3 rounded-lg flex items-center justify-between transition-all duration-300 hover:bg-gray-100 hover:shadow-md">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-600 font-medium transition-all duration-300 hover:text-gray-800">
                        Expires:{" "}
                        {calculateExpiresAt(user.createdAt, user.subscription)}
                      </span>
                    </div>
                  </div>
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-600 px-3 py-1 bg-gray-100 rounded-full transition-all duration-300 hover:bg-gray-200">
                Page {currentPage} of{" "}
                {Math.ceil(filteredData.length / postPerPage)}
              </span>
              <Button
                variant="outline"
                size="sm"
                ...
                disabled={
                  currentPage === Math.ceil(filteredData.length / postPerPage)
                }
                className="transition-all duration-300 hover:scale-105 active:scale-95 disabled:opacity-50"
              >
                Next

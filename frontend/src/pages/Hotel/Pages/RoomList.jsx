...
function RoomList() {
  ...
  const fetchRooms= useCallback(
    async (pageNumber = 1, searchTerm = "") => {
      ...
      try {
        ...
        const res = await api.get(`/api/sUsers/getRooms/${cmp_id}?${params}`, {
        ...
        if (pageNumber === 1) {
          ...
          setRooms((prevRooms) => [...prevRooms, ...res?.data?.roomData]);
        }
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation.isConfirmed) {
      setLoader(true);
      try {
        const res = await api.delete(`/api/sUsers/deleteRoom/${id}`, {
          headers: {
  ...
  const isItemLoaded = (index) => index < rooms.length;
  ...
  const Row = ({ index, style }) => {
    ...
    return (
      ...
      >
        <div className="">
          <p className="font-bold text-sm">{el?.roomName}</p>
        </div>
        <hr className="mt-4" />
  ...
  return (
    <>
      ...
      <div className="flex-1 bg-slate-50  h-screen overflow-hidden  ">
        ...
        {!loader && !isLoading && rooms?.length === 0 && (
        ...
        <div className="pb-4">
          <InfiniteLoader
            ...
            itemCount={hasMore ? rooms?.length + 1 : rooms?.length}

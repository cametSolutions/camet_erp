    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (response.data.checkoutBills.length === 0) {
          setError(`No checkout data found for ${formatDate(selectedDate)}`);
        }
      } else {
      ...
      setError(`Failed to load checkout data: ${err.response?.data?.message || err.message}`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="border-2 border-black p-6">
        ...
        <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-400">
          ...
          <div className="text-sm">
            <span className="font-semibold">Print Date & Time:</span> {getCurrentDateTime()}
          </div>
        </div>

        <div className="text-sm mb-4">
          <span className="font-semibold">FO Cash Statement From</span> {formatDate(selectedDate)} 
          <span className="font-semibold"> To</span> {formatDate(selectedDate)}
        </div>
          ...
          <tbody>
            <tr className="border-b border-gray-300">
              <td colSpan="11" className="py-2 px-1 font-semibold">
                Checkout Bills ({checkoutData.length} {checkoutData.length === 1 ? 'record' : 'records'})
              </td>
            </tr>
              ...
              <tr>
                <td colSpan="11" className="text-center py-8 text-gray-500">
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-sm mt-1">for {formatDate(selectedDate)}</p>
                  </div>
                </td>
              </tr>
            )}
            {checkoutData.length > 0 && (
              <tr className="border-t-2 border-black font-semibold bg-gray-50">
                <td colSpan="6" className="py-2 px-1">Total Amount</td>
                <td className="py-2 px-1">{(summary?.cardTotal || 0).toFixed(2)}</td>
                <td></td>
                <td></td>
                <td className="text-right py-2 px-1">{totalAmount.toFixed(2)}</td>
                <td></td>
              </tr>
            )}
          </tbody>

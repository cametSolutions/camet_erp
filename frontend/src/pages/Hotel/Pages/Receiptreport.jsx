...
import { setReceiptReportDate } from "../../../../slices/dateSlice";
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
        ...
        <div style={{ padding: "16px 20px", borderRight: "1px solid #f1f5f9" }}>
          ...
          >
            <span style={{ fontSize: 13, fontWeight: 800, color: "#1a3a5c" }}>
              Net Total
            </span>
            <span
              style={{
        ...
        <div style={{ padding: "16px 20px" }}>
          ...
          >
            * This report shows only receipt transactions.
            <br />* Cancelled receipts are excluded.
          </div>
        </div>
      </div>
      ...
      >
        {paymentCards
          ...
          .map((p) => (
            <div
              key={p.key}
              style={{ padding: "3px 16px", borderRight: "1px solid #e2e8f0" }}
            >
              <div
                style={{
                  fontSize: 13,
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 140 }}>
        ...
        {[
          ...
          .filter(([, v]) => Number(v) > 0)
          .map(([l, v]) => (
            <div key={l} style={eRow}>
              <span style={eL}>{l}</span>
              <span style={eV}>₹{fmt(v)}</span>
            </div>
          ))}
      </div>
      ...
      <div style={{ flex: 1, minWidth: 200 }}>
        ...
        <div style={eRow}>
          <span style={eL}>Party</span>
          <span style={eV}>{r.partyName || "—"}</span>
        </div>
        <div style={eRow}>
          <span style={eL}>Total Bill Amt</span>
          <span style={eV}>₹{fmt(r.totalBillAmount)}</span>
        </div>
        <div style={eRow}>
        ...
        {r.advanceAmount > 0 && (
          <div style={eRow}>
            <span style={eL}>Advance</span>
            <span style={eV}>₹{fmt(r.advanceAmount)}</span>
          </div>
        )}
        {r.remainingAmount > 0 && (
          <div style={eRow}>
            <span style={eL}>Remaining</span>
            <span style={{ ...eV, color: "#dc2626" }}>
        ...
        <div style={{ flex: 1, minWidth: 220 }}>
          ...
          {r.billDataList.map((b, idx) => (
            ...
            >
              ...
              <div style={eRow}>
                <span style={eL}>Bill Date</span>
                <span style={eV}>{fmtDate(b.billDate)}</span>
              </div>
              <div style={eRow}>
      ...
      {r.billType === "restaurant" && (
        <div style={{ flex: 1, minWidth: 160 }}>
          ...
          <div style={eRow}>
            <span style={eL}>Table</span>
            <span style={eV}>{r.tableNumber || "—"}</span>
          </div>
          <div style={eRow}>
            <span style={eL}>Waiter</span>
            <span style={eV}>{r.waiterName || "—"}</span>
          </div>
          <div style={eRow}>
            <span style={eL}>KOT No</span>
            <span style={eV}>{r.kotNumber || "—"}</span>
          </div>
          <div style={eRow}>
            <span style={eL}>KOT Type</span>
            <span style={eV}>{r.kotType || "—"}</span>
          </div>
        </div>
      )}
    </div>
...
export default function ReceiptReport() {
  ...
  const receiptReportDate = useSelector(
    (state) => state.selectedDate.receiptReportDate,
  );

  const { start, end, autoFetch } = receiptReportDate;
 ...
 useEffect(() => {
  ...
  const rows = useMemo(() => {
    console.log(
      "ALL RECEIPTS billTypes:",
      allReceipts.map((r) => ({ billNo: r.billNo, billType: r.billType })),
    );
    const q = search.toLowerCase().trim();
    return allReceipts.filter((r) => {
      if (filter === "restaurant" && r.billType === "hotel") return false;
      if (filter === "hotel" && r.billType?.toLowerCase() !== "hotel")
        return false;
      if (!q) return true;

      const billMatch =
        r.billDataList?.some((b) => b.billNo?.toLowerCase().includes(q)) ||
        false;
      return (
    ...
    () =>
      rows.reduce(
        (a, r) => {
          ...
          if (r.billType === "hotel") {
            a.hCount++;
            a.hTotal += r.finalAmount || 0;
          }
          if (r.billType === "restaurant") {
            a.rCount++;
            a.rTotal += r.finalAmount || 0;
          }
          return a;
        },
        {
  ...
  const exportToPDF = () => {
    ...
    doc.text(
      `Period: ${fromDate} to ${toDate}   |   Filter: ${filter}   |   ${rows.length} receipts`,
      14,
      20,
    );
    const head = [
    ...
    <>
      ...
      >
        ...
        <div className="mb-4 rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="p-3 md:p-4">
            ...
            <div className="flex flex-wrap items-end gap-2">
              ...
              <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
                ...
                <input
                  type="text"
                  placeholder="Bill No, Party, Room, Table…"
                  value={search}
          ...
          {loading && (
            <div
              style={{ height: 3, background: "#e2e8f0", overflow: "hidden" }}
            >
        ...
        <div className="mb-3 flex gap-2 flex-wrap">
          {[
            { val: "all", label: "🏨 + 🍽 All", count: allReceipts.length },
            {
              val: "hotel",
              label: "🏨 Hotel",
              count: allReceipts.filter((r) => r.billType === "hotel").length,
            },
            {
              val: "restaurant",
              label: "🍽 Restaurant",
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        <td style={tdBase}>
                          <SourceBadge source={r.receiptSource} />
                        </td>
                        <td style={tdBase}>
                          {(r.billDataList || []).length > 0 ? (
                            <div
                        ...
                        <td
                          style={{ ...tdBase, color: "#64748b", fontSize: 12 }}
                        >
                          {(r.billDataList || []).length > 0 ? (
                            <div
                              style={{

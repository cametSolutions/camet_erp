...
import { X, Clock, Calendar, TrendingUp, TrendingDown, Minus } from 'lucide-react';

export default function TariffHistory({ isOpen, onClose, booking, formData }) {
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-[#1e3a5f] to-[#2d5278]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Tariff Rate History</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
          {tariffHistory.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-gray-500 bg-white rounded-lg border border-gray-200">
              ...
              <p className="text-lg font-semibold text-gray-700">No Tariff History Available</p>
              <p className="text-sm mt-2 text-gray-500">Rate changes will appear here</p>
            </div>
          ) : (
            <div className="space-y-3">
              {tariffHistory.map((entry, index) => {
                const previousEntry = tariffHistory[index + 1];
                const change = previousEntry ? calculateChange(entry.rate, previousEntry.rate) : null;
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className={`p-2.5 rounded-lg ${isCurrentDate ? 'bg-orange-100' : 'bg-gray-100'}`}>
                          <Calendar size={20} className={isCurrentDate ? 'text-orange-600' : 'text-gray-600'} />
                        </div>
                        <div>
                          ...
                          {isCurrentDate && (
                            <p className="text-xs text-orange-600 font-medium mt-0.5">Selected Date</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        {change && (
                          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-semibold ${
                            ...
                            <span>
                              {change.change > 0 ? '+' : ''}₹{Math.abs(change.change).toFixed(0)}
                            </span>
                        ...
                        <div className="text-right">
                          <p className={`text-2xl font-bold ${isCurrentDate ? 'text-orange-600' : 'text-gray-900'}`}>
                    ...
                    {index < tariffHistory.length - 1 && (
                      <div className={`absolute left-[2.85rem] top-full w-0.5 h-3 ${
        ...
        <div className="px-6 py-4 border-t bg-white flex justify-between items-center">
          <div className="text-sm text-gray-600">
            <span className="font-semibold text-gray-800">{tariffHistory.length}</span> rate {tariffHistory.length === 1 ? 'change' : 'changes'} recorded
          </div>
          <button
            onClick={onClose}

...
export default function PaymentAllocation({
  ...
  const discountAmount = useMemo(() => {
    if (!selectedItem || discountValue === "" || isNaN(Number(discountValue)))
      return 0;
    const base = selectedItem.subTotal;
    if (discountType === "percentage") {
      return +(
        (base * Math.min(Math.max(Number(discountValue), 0), 100)) /
        100
      ).toFixed(2);
    }
    return +Math.min(Math.max(Number(discountValue), 0), base).toFixed(2);
  ...
  const handleConfirm = () => {
    if (canConfirm)
      onConfirm({ selectedItem, discountAmount, netPayable, balance });
  };
    ...
    <div style={S.overlay} onClick={onClose}>
      <div style={S.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ overflowX: "auto", position: "relative" }}>
          {loading ? (
            ...
            <table style={S.table}>
              ...
              <tbody>
                {saleData.length === 0 ? (
                  ...
                  saleData.map((item) => {
                    ...
                    const net = +(item.subTotal - (isSel ? discountAmount : 0)).toFixed(2);
                    return (
                      <tr
                        key={item.id}
                          ...
                          {isSel ? (
                            <div style={S.discWrap}>
                              <div style={S.toggleGroup}>
                                <button
                                  style={{ ...S.toggleBtn, ...(discountType === "percentage" ? S.toggleOn : {}) }}
                                  onClick={() => { setDiscountType("percentage"); setDiscountValue(""); }}
                                >%</button>
                                <button
                                  style={{ ...S.toggleBtn, ...(discountType === "amount" ? S.toggleOn : {}) }}
                              ...
                              />
                              {discountAmount > 0 && (
                                <span style={S.discBadge}>−{fmt(discountAmount)}</span>
                              )}
                            </div>
                          ) : (
              ...
              <tfoot>
                <tr>
                  <td colSpan={2} style={S.tfoot}>{saleData.length} items</td>
                  <td style={{ ...S.tfoot, textAlign: "right" }}>{fmt(totalRaw)}</td>
                  <td style={S.tfoot} />
                  <td style={{ ...S.tfoot, textAlign: "right", color: "#1a4fa0" }}>{fmt(totalNet)}</td>
                </tr>
              </tfoot>
            </table>
          )}
        ...
        <div style={S.summary}>
          ...
          <SumTile label="Item Amount" value={selectedItem ? fmt(selectedItem.subTotal) : "—"} />
          <SumTile
            label="Discount"
          ...
          <SumTile
            ...
            valueColor={balance < 0 ? "#c0280a" : balance === 0 ? "#1a8a4a" : "#0d1b2e"}
            last
          />
        </div>
...
const S = {
  ...
  headIcon: {
    background: "#1a6fc4", borderRadius: 6, width: 30, height: 30,
    display: "flex", alignItems: "center", justifyContent: "center",
  },
  headTitle: { color: "#fff", fontSize: 14, fontWeight: 600 },
  headSub:   { color: "#7a99bb", fontSize: 11, marginTop: 1 },
  closeBtn: {
    background: "rgba(255,255,255,.1)", border: "none", borderRadius: 5,
    width: 26, height: 26, cursor: "pointer", color: "#aac4e0", fontSize: 14,
  },
  infoBar: {
    background: "#f0f6ff", borderBottom: "1px solid #dce8f5", display: "flex",
  },
  infoChip: {
    display: "flex", flexDirection: "column",
    padding: "9px 16px", borderRight: "1px solid #dce8f5",
  },
  infoChipLast: { borderRight: "none", marginLeft: "auto", alignItems: "flex-end" },
  icLabel: {
    fontSize: 10, fontWeight: 600, color: "#5a7a9a",
  ...
  loaderText: {
    fontSize: 12, color: "#5a7a9a", fontWeight: 600,
  ...
  skeletonCell: {
    height: 12, borderRadius: 4, background: "#e8f0fb",
  ...
  emptyCell: {
    ...
    fontSize: 12, color: "#8aa4c0", fontStyle: "italic",
  },
  ...
  sBadge: {
    ...
    fontSize: 11, fontWeight: 600, background: "#f0e8fb", color: "#5a1aa0",
  },
  discWrap:    { display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" },
  toggleGroup: {
    display: "flex", border: "1px solid #c5d5e8",
    borderRadius: 5, overflow: "hidden",
  },
  toggleBtn: {
    padding: "2px 8px", border: "none", background: "#f5f8fc",
    cursor: "pointer", fontSize: 11, fontWeight: 600,
    color: "#4a6a8a", fontFamily: "inherit",
  },
  toggleOn:  { background: "#1a6fc4", color: "#fff" },
  discInput: {
    width: 66, padding: "3px 6px",
    border: "1px solid #c5d5e8", borderRadius: 5,
    fontSize: 11, fontFamily: "inherit", color: "#1a2740", outline: "none",
  },
  discBadge: {
    fontSize: 10, fontWeight: 700, color: "#c0280a",
    background: "#fff0ed", padding: "1px 5px", borderRadius: 4,
  },
  tfoot: {
    padding: "7px 10px", fontSize: 11, fontWeight: 700,
    color: "#5a7a9a", background: "#f5f8fc", borderTop: "1.5px solid #dce8f5",
  },
  alert: {
    background: "#fff0ed", borderTop: "1px solid #f5c0b0",
    padding: "6px 16px", fontSize: 11, fontWeight: 600, color: "#c0280a",
  },
  summary: {
    background: "#f0f6ff", borderTop: "1px solid #dce8f5",
    display: "flex", alignItems: "center", flexWrap: "wrap",
  },
  sumTile: {
    display: "flex", flexDirection: "column",
    padding: "9px 14px", borderRight: "1px solid #dce8f5",
  },
  ...
  footer: {
    background: "#fff", borderTop: "1px solid #eef2f7",
    display: "flex", justifyContent: "flex-end", alignItems: "center",
    gap: 8, padding: "10px 16px",
  },
  btnCancel: {
    padding: "6px 16px", border: "1px solid #c5d5e8", borderRadius: 5,
    background: "#fff", color: "#4a6a8a", fontSize: 12, fontWeight: 600,
    cursor: "pointer", fontFamily: "inherit",
  },
  btnPay: {
    padding: "6px 18px", border: "none", borderRadius: 5,
    background: "#1a6fc4", color: "#fff", fontSize: 12, fontWeight: 700,
    cursor: "pointer", fontFamily: "inherit",
  },

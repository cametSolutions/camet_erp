...
const DashboardTransaction = ({ filteredData, userType, from }) => {
  ...
  const getNavigationPath = useMemo(() => (type, id) => {
  ...
  const renderTransaction = useMemo(() => (transaction, index) => {
    const { type, _id, party_name, billNo, createdAt, enteredAmount, isCancelled } = transaction;
    ...
    return (
      ...
      >
        <div className="flex justify-start text-xs mt-2">
          <div className={`${typeColors[type] || typeColors.default} flex items-center text-white px-2 rounded-sm`}>

...
export const convertCheckOutToSale = async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const specificVoucherSeries = await hotelVoucherSeries(cmp_id, session, "sales", false);
      ...
      const [allBookings, allCheckins] = await Promise.all([
        Booking.find({ voucherNumber: { $in: bookingVoucherNumbers }, cmp_id }).session(session),
        CheckIn.find({ voucherNumber: { $in: checkinVoucherNumbers }, cmp_id }).session(session),
      ]);
      ...
      for (const item of selectedCheckOut) {
        ...
        if (paymentMode === "single") {
          ...
          applicableSplits = splitDetails.map((splitItem) => {
            if (splitItem.sourceType === "credit") {
              return { ...splitItem, source: splitItem.customer, sourceType: "cash" };
            }
            return splitItem;
          });
        ...
        const checkOutDoc = await CheckOut.create([{
          ...
          checkoutType:
            checkoutMode === "single" ? "singleCheckout" : "individualCheckout",
        }], { session });
        ...
        const amount = [item].reduce((total, el) => {
          return total + (el.selectedRooms || []).reduce(
...
console.log("splitEntry",splitEntry)
            const rows = await createTallyEntry(
              ...
              isCredit ? Number(splitEntry.amount || 0) : Number(splitEntry.amount || 0),    // bill_pending_amt
              session,
              "checkout",
            );
      ...
      if (!isPostToRoom && paymentMode !== "credit") {
        ...
        await createReceiptsAndSettlements({
          ...
          specificVoucherSeriesRestaurant: specificVoucherSeriesReceiptRestaurant,
          req,
          session,
        });
      }
    ...
    if (createdCheckoutIds.length > 0) {
      checkOutAfterSave = await CheckOut.find({ _id: { $in: createdCheckoutIds } })
        .populate("customerId")
        .populate("guestId")
        .populate("agentId")
...
export const handleAdvanceAndDiscountSettlementInRestaurant = async (
  ...
  try {
    ...
    const tallyData  = await TallyData.find({ billId: { $in: checkInIds } }).session(session);

    const restaurantSideDiscountAdjustmentArray = settlementData;

    let totalOfDiscountAndAdvance = restaurantSideDiscountAdjustmentArray.reduce(
      (acc, item) =>
...
const specificVoucherSeries = SaleVoucher.series.find(
  (series) =>
    series.under === (isRestaurant ? "restaurant" : "hotel")
);
...
async function createPaymentSplittingArray(
  ...
  if (paymentMode === "split" && applicableSplits.length > 0) {
    // ── SPLIT MODE ─────────────────────────────────────────────────────────
    // underCategory="room" → hotel arr
    // underCategory="food" → restaurantSplitArray
    for (const split of applicableSplits) {
      let isCredit = split.subsource === "credit";
      const splitObj = {
        ...
        source:        isCredit ? getCustomerId(split.customer) : split.source,
        sourceType:    isCredit ? "credit" : split.sourceType,
        subsource:     isCredit ? "credit" : split.subsource,
        transactionNo: split.transactionNo,
        underCategory: split.underCategory,
        upiNo:         split.upiNo,
      };

      if (split.underCategory === "room") arr.push(splitObj);
      else restaurantSplitArray.push(splitObj);
    }

  } else if (paymentMode === "credit") {
    // ── CREDIT MODE ────────────────────────────────────────────────────────
    // FIX 1: cashAmt = 0 in credit mode (set by loop), so read total directly
    // FIX 2: Do NOT access splitDetails[0] — it is empty in credit mode
    ...
    // ── SINGLE MODE ────────────────────────────────────────────────────────
    // FIX 3: splitDetails may be empty in single mode — guard with fallback {}
  ...
  if (restaurantBaseSaleData.length > 0 && restaurantTotal > 0) {
    ...
    if (paymentMode === "split" && restaurantSplitArray.length > 0) {
      ...
      // FIX 4: Do NOT access splitDetails[0] — build from selectedCreditor directly
      const creditorId   = paymentDetails?.selectedCreditor?._id;
      const creditorName = paymentDetails?.selectedCreditor?.partyName;
      ...
      // Single mode: restaurantSplitArray already built above
      splitsToDistribute = restaurantSplitArray.map((s) => ({ ...s }));
    }
    ...
    if (restaurantBaseSaleData.length === 1) {
      ...
      // Multiple restaurant sales — greedy sequential distribution
      const updatePromises = [];
      let remainingSplits  = splitsToDistribute;

      for (const saleItem of restaurantBaseSaleData) {
        let remaining    = saleItem.finalAmount;
        const itemSplits = [];
...
async function createSalesVoucher(
  ...
  const AlreadyExistingItems = selectedCheckOut.flatMap((item) => item.selectedRooms);
  let items = [];

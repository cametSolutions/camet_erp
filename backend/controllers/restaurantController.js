  ...
  try {
    ...
    const categorizeSales = (sales) => {
      const result = {
        hotel: { 
          ...
          expense: 0, itemCount: 0, transactionCount: 0
        },
        restaurant: { 
          totalSales: 0, cashReceipt: 0, bankReceipt: 0, upiAmount: 0,
          chequeAmount: 0, creditAmount: 0, totalTax: 0, totalDiscount: 0,
          ...
          sale.items.forEach(item => {
            if (item.GodownList && Array.isArray(item.GodownList)) {
              item.GodownList.forEach(godown => {
                totalTax += Number(godown.cgstAmt || 0) + Number(godown.sgstAmt || 0) + Number(godown.igstAmt || 0);
    ...
    const calculateAnalytics = (combined) => ({
      paymentMethodBreakdown: {
        cashPercentage: combined.totalSales > 0 ? 
          (combined.cashReceipt / combined.totalSales) * 100 : 0,
        bankPercentage: combined.totalSales > 0 ? 
          (combined.bankReceipt / combined.totalSales) * 100 : 0,
        creditPercentage: combined.totalSales > 0 ? 
          (combined.creditAmount / combined.totalSales) * 100 : 0,
        upiPercentage: combined.totalSales > 0 ? 
          (combined.upiAmount / combined.totalSales) * 100 : 0
      },
      businessMix: {
        hotelPercentage: combined.totalSales > 0 ? 
          (dailyData.hotel.totalSales / combined.totalSales) * 100 : 0,
        restaurantPercentage: combined.totalSales > 0 ? 
          (dailyData.restaurant.totalSales / combined.totalSales) * 100 : 0,
      }
    });
